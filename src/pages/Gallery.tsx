    ...
    <div className="py-12 sm:py-16 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <p className="text-center text-gray-500 max-w-xl mx-auto mb-10 -mt-4">
          A glimpse into life at Masakala SSS — from the classroom to the sports field, from assembly to the mountain backdrop.
        </p>
        ...
        <div className="flex flex-wrap gap-2 justify-center mb-8">
          {categories.map(cat => (
            <button
              ...
              className={`px-4 py-2 rounded-full text-sm font-bold transition-all border-2 ${activeCategory === cat ? 'bg-school-green text-[#FED219] border-school-green' : 'bg-white text-school-green border-school-green hover:bg-[#FED219]/10'}`}
            >
              {cat}
            </button>
          ))}
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
          {filtered.map((img, i) => (
            ...
            >
              ...
              <span className="absolute top-2 right-2 bg-school-green text-[#FED219] text-xs px-2 py-0.5 rounded-full font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
                {img.category}
              </span>
            </motion.div>
          ))}
